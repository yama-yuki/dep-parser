...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        #with open('id2tran.pkl', 'rb') as f:
            #self.id2tran = pickle.load(f)
    ...
    def parse_step(self, transition):
        ...
        #transition = self.id2tran[transition]
        #print(transition)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for i, (transition, parse) in enumerate(zip(transitions, batch_parses)):
            parse.parse_step(transition)
            print(parse.buffer)
            if len(parse.buffer) == 0 and len(parse.stack) == 1:
                finished_parses_idx.append(i)
        unfinished_parses = [unfinished_parses[i] for i in range(len(unfinished_parses)) if i not in finished_parses_idx]
