...
class Config(object):
    ...
    input_path = '../data/inout'
    input_file = 'tagged.txt'
    embedding_file = '../data/original/en-cw.txt'
    model_name = '20211214_234148' #'20211210_151924'
...
class Parser(object):
    ...
    def vectorize(self, examples, snt=False):
        if snt:
            vec_examples = []
            for ex in examples:
    ...
    def parse(self, dataset, eval_batch_size=5000, eval=True):
...
def read_input(in_file, lowercase=False):
    examples = []
...
def minibatches(data, batch_size, n_trans):
    #print(len(data[0][1]))
    ...
    one_hot = np.zeros((y.size, n_trans))
    #print(one_hot.shape)
    one_hot[np.arange(y.size), y] = 1
    return get_minibatches([x, one_hot], batch_size)


def load_and_preprocess_data(reduced=True, mode='Train'):
    config = Config()

    if mode == 'Parse':
        train_set = read_conll(os.path.join(config.data_path, config.train_file),
                            lowercase=config.lowercase)
        input_set = read_input(os.path.join(config.input_path, config.input_file),
                            lowercase=config.lowercase)
        ...
        input_set = parser.vectorize(input_set, snt=True)
        test_set = parser.vectorize(test_set)

        return parser, embeddings_matrix, test_set, input_set, model_name
